...
//        YangJavaModule yangJavaModule = (YangJavaModule)this.getYangJavaModule();
        ...
        List<JavaField> javaFieldList = this.getEnumSet().stream().map(yangEnum -> new JavaField("String", YangElement.normalizeClass(yangEnum.getNamedValue()
                .matches("^[0-9]+[\\s\\S]*") ? "J" + yangEnum.getNamedValue() : yangEnum.getNamedValue()) .replace("/","").replace("\\\\","").replace("*","X"),
