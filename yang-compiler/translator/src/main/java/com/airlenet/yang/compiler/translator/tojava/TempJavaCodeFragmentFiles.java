...
public class TempJavaCodeFragmentFiles {
    ...
    public TempJavaCodeFragmentFiles(JavaFileInfoTranslator javaFileInfo)
        ...
        if ((genType & GeneratedJavaFileType.GENERATE_INTERFACE_WITH_BUILDER) != 0) {
            beanTempFiles = new TempJavaBeanFragmentFiles(javaFileInfo);
        }

        if ((genType & GeneratedJavaFileType.GENERATE_TYPE_CLASS) != 0) {
            typeTempFiles = new TempJavaTypeFragmentFiles(javaFileInfo);
        }

        if ((genType & GeneratedJavaFileType.GENERATE_ENUM_CLASS) != 0) {
            enumTempFiles = new TempJavaEnumerationFragmentFiles(javaFileInfo);
        }

        if ((genType & GeneratedJavaFileType.GENERATE_SERVICE_AND_MANAGER) != 0) {
            serviceTempFiles = new TempJavaServiceFragmentFiles(javaFileInfo);
        }

        if ((genType & GeneratedJavaFileType.GENERATE_ALL_EVENT_CLASS_MASK) != 0) {
            eventTempFiles = new TempJavaEventFragmentFiles(javaFileInfo);
        }

        if ((genType & GeneratedJavaFileType.GENERATE_ALL_RPC_CLASS_MASK) != 0) {
            rpcTempFiles = new TempJavaRpcFragmentFiles(javaFileInfo);
        }

        if ((genType & GeneratedJavaFileType.GENERATE_RPC_COMMAND_CLASS) != 0) {
    ...
    public void generateJavaFile(int fileType, YangNode curNode)
        ...
        if ((fileType & GeneratedJavaFileType.GENERATE_INTERFACE_WITH_BUILDER) != 0) {
         ...
         */
        if ((fileType & GeneratedJavaFileType.GENERATE_TYPE_CLASS) != 0) {
         ...
         */
        if (fileType == GeneratedJavaFileType.GENERATE_SERVICE_AND_MANAGER) {
            serviceTempFiles.generateJavaFile(GeneratedJavaFileType.GENERATE_SERVICE_AND_MANAGER, curNode);
        }

        /*
         * Creates event, event listener and event subject files.
         */
        if (fileType == GeneratedJavaFileType.GENERATE_ALL_EVENT_CLASS_MASK) {
            eventTempFiles.generateJavaFile(GeneratedJavaFileType.GENERATE_ALL_EVENT_CLASS_MASK, curNode);
        }

        /*
         * Creates enumeration class file.
         */
        if (fileType == GeneratedJavaFileType.GENERATE_ENUM_CLASS) {
            enumTempFiles.generateJavaFile(GeneratedJavaFileType.GENERATE_ENUM_CLASS, curNode);
        }

        /*
         * Creates RPC files.
         */
        if (fileType == GeneratedJavaFileType.GENERATE_ALL_RPC_CLASS_MASK) {
            rpcTempFiles.generateJavaFile(GeneratedJavaFileType.GENERATE_ALL_RPC_CLASS_MASK, curNode);
        }

        /*
         * Creates RPC command file.
         */
        if (fileType == GeneratedJavaFileType.GENERATE_RPC_COMMAND_CLASS) {
            rpcCommandTempFiles.generateJavaFile(GeneratedJavaFileType.GENERATE_RPC_COMMAND_CLASS, curNode);
        }
    }
