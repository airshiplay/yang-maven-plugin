...
//        }
    ...
    public void generateCodeExit() throws TranslatorException {
        ...
        YangType typeInTypeDef = this.getTypeDefBaseType();
        String typeInfo = JNCCodeUtil.getInfoByYangType(typeInTypeDef);
        ...
        if (typeInTypeDef.getDataType() == ENUMERATION) {
            ...
            List<JavaField> javaFieldList = ((YangJavaEnumeration) this.getTypeList().get(0).getDataTypeExtendedInfo())
                    .getEnumSet().stream().map(yangEnum -> new JavaField("String",
                            YangElement.normalizeClass(yangEnum.getNamedValue().matches("^[0-9]+[\\s\\S]*") ? "J" + yangEnum.getNamedValue() : yangEnum.getNamedValue())
                                    .replace("/","").replace("*","X"),
