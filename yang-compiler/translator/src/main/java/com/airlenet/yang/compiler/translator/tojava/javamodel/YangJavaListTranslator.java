...
///constructor
        if (!keys.isEmpty()) {
            ...
            if (parentYangJavaModule == null || getParent() instanceof YangJavaModule || !parentYangJavaModule.getPrefixClassName().equals(yangJavaModule.getPrefixClassName())) {
                constructor.addLine("setDefaultPrefix();");
            }
            if (getParent() != null && getParent() instanceof YangJavaAugment) {
                constructor.addLine("setDefaultPrefix();");
//            emptyConstructorMethod.addLine("setPrefix("+parentYangJavaModule.getPrefixClassName()+".PREFIX);");
...
//                    constructor.addLine("Leaf " + YangElement.camelize(yangLeaf.getName()) + " = new Leaf(" + javaModule.getJavaPackage() + "." + javaModule.getPrefixClassName() + ".NAMESPACE, \"" + yangLeaf.getName() + "\");");

                    constructor.addLine( YangElement.camelize(yangLeaf.getName())+"Value.getValue().setPrefix(new com.tailf.jnc.Prefix("+
            ...
            for (YangLeaf yangLeaf : keys) {
                ...
                builder.append(" new " + AttributesJavaDataType.getJNCDataType(dataTypeYangLeaf.getDataType())
                        + "(" + YangElement.camelize(yangLeaf.getName() + "Value") + ")");
                builder.append(",");
            }
            builder.deleteCharAt(builder.length()-1);
            builder.append(");");
            constructor.addLine(builder.toString());
            constructor.setModifiers("public").setExceptions("JNCException").addDependency(yangJavaModule.getJavaPackage() + "." + yangJavaModule.getPrefixClassName());
            javaClass.addMethod(constructor);
