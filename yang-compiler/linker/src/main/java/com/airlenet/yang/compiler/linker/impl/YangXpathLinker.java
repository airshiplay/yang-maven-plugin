...
import com.airlenet.yang.compiler.datamodel.LeafRefInvalidHolder;
...
import static com.airlenet.yang.compiler.utils.UtilConstants.ERROR_MSG_FOR_AUGMENT_LINKING;
...
import static com.airlenet.yang.compiler.utils.UtilConstants.LEAF_HOLDER_ERROR;
...
public class YangXpathLinker<T> {
    ...
    public YangNode processUsesAugLinking(List<YangAtomicPath> paths,
        ...
        absPaths = paths;
        rootNode = uses;
        linkingType = XpathLinkingTypes.USES_AUGMENT_LINKING;
    ...
    private void validateTempPathNode(YangNode node) {
        ...
        if (linkingType != XpathLinkingTypes.AUGMENT_LINKING) {
            return;
        }
    ...
    private YangNode getImportedNode(YangNode root, YangNodeIdentifier nodeId) {
        ...
        for (YangImport imported : importList) {
            ...
            if( imported.getImportedNode() instanceof YangModule){
                YangModule importedNode = (YangModule) imported.getImportedNode();
                if(importedNode.getPrefix().equals(nodeId.getPrefix())){
                    return importedNode;
                }
            }
        }
        ...
        while (pathIterator.hasNext()) {
            ...
            if (curPrefix != null && !(curPrefix.equals(rootPrefix))) {
                if (!curPrefix.equals(prePrefix)) {
                    if (prePrefix != null) {
                        prefixResolverTypes.put(absPath, PrefixResolverType.INTER_TO_INTER);
                    } else {
                        prefixResolverTypes.put(absPath, PrefixResolverType.INTRA_TO_INTER);
                    }
                } else {
                    prefixResolverTypes.put(absPath, PrefixResolverType.NO_PREFIX_CHANGE_FOR_INTER);
                }
            } else {
                if (prePrefix != null) {
                    prefixResolverTypes.put(absPath, PrefixResolverType.INTER_TO_INTRA);
                } else {
                    prefixResolverTypes.put(absPath, PrefixResolverType.NO_PREFIX_CHANGE_FOR_INTRA);
                }
            }
        }
    }
