...
import static com.airlenet.yang.compiler.datamodel.YangStatusType.CURRENT;
 ...
 */
public abstract class YangTailfAction
        extends YangNode
     ...
     * Creates a rpc node.
     */
    public YangTailfAction() {
        super(YangNodeType.RPC_NODE, new HashMap<>(), DataNode.Type.SINGLE_INSTANCE_NODE);
    ...
    public void detectSelfCollision(String identifierName,
        ...
        if (getName().equals(identifierName)) {
            throw new DataModelException(ErrorMessages.getErrorMsgCollision(
                    ErrorMessages.COLLISION_DETECTION, getName(), getLineNumber(),
                    getCharPosition(), ErrorMessages.RPC, getFileName()));
        }
    }
    ...
    public SchemaContext getChildContext(SchemaId schemaId) {
        ...
        checkNotNull(schemaId, DataModelUtils.E_ID);
        YangSchemaNodeIdentifier id = DataModelUtils.getNodeIdFromSchemaId(
        ...
        try {
            ...
            if (node instanceof SchemaDataNode) {
                ...
                throw new IllegalArgumentException(DataModelUtils.errorMsg(DataModelUtils.FMT_NOT_EXIST,
                                                            schemaId.name(),
                                                            getName()));
            }
