...
import static com.airlenet.yang.compiler.datamodel.utils.YangConstructType.UNION_DATA;
import static com.airlenet.yang.compiler.parser.antlrgencode.GeneratedYangParser.UnionSpecificationContext;
import static com.airlenet.yang.compiler.translator.tojava.YangDataModelFactory.getYangUnionNode;
...
public final class UnionListener {
    ...
    public static void processUnionEntry(TreeWalkListener listener,
        ...
        ListenerValidation.checkStackIsNotEmpty(listener, ListenerErrorType.MISSING_HOLDER, UNION_DATA, "", ListenerErrorLocation.ENTRY);
            ...
            switch (tmpData.getYangConstructType()) {
                ...
                default:
                    throw new ParserException(ListenerErrorMessageConstruction.constructListenerErrorMessage(ListenerErrorType.INVALID_HOLDER, TYPE_DATA,
                            ((YangType<?>) typeData).getDataTypeName(), ListenerErrorLocation.ENTRY));
            }
            listener.getParsedDataStack().push(typeData);
            listener.getParsedDataStack().push(unionNode);
        } else {
            throw new ParserException(ListenerErrorMessageConstruction.constructListenerErrorMessage(ListenerErrorType.INVALID_HOLDER, UNION_DATA, "", ListenerErrorLocation.ENTRY));
        }
    }
    ...
    public static void processUnionExit(TreeWalkListener listener,
        ...
        // Check for stack to be non empty.
        ListenerValidation.checkStackIsNotEmpty(listener, ListenerErrorType.MISSING_HOLDER, UNION_DATA, "", ListenerErrorLocation.EXIT);
            ...
            switch (tmpNode.getYangConstructType()) {
                ...
                default:
                    throw new ParserException(
                            ListenerErrorMessageConstruction.constructListenerErrorMessage(ListenerErrorType.INVALID_HOLDER, UNION_DATA, "", ListenerErrorLocation.EXIT));
            }
        } else {
            throw new ParserException(
                    ListenerErrorMessageConstruction.constructListenerErrorMessage(ListenerErrorType.MISSING_CURRENT_HOLDER, UNION_DATA, "", ListenerErrorLocation.EXIT));
        }
    }
    ...
    private static void addChildToParentNode(TreeWalkListener listener, YangUnion unionNode) {
        if (!(listener.getParsedDataStack().peek() instanceof YangNode)) {
            throw new ParserException(ListenerErrorMessageConstruction.constructListenerErrorMessage(ListenerErrorType.INVALID_HOLDER, UNION_DATA,
            ...
            try {
                ...
                throw new ParserException(ListenerErrorMessageConstruction.constructExtendedListenerErrorMessage(ListenerErrorType.UNHANDLED_PARSED_DATA,
                                                                                UNION_DATA, "", ListenerErrorLocation.ENTRY, e.getMessage()));
            }
        }
    }
}
