...
import static com.airlenet.yang.compiler.datamodel.utils.YangConstructType.INCLUDE_DATA;
...
public final class IncludeListener {
    ...
    public static void processIncludeEntry(TreeWalkListener listener, IncludeStatementContext ctx) {
        ...
        ListenerValidation.checkStackIsNotEmpty(listener, ListenerErrorType.MISSING_HOLDER, INCLUDE_DATA, ctx.identifier().getText(),
                ListenerErrorLocation.ENTRY);

        String identifier = ListenerUtil.getValidIdentifier(ctx.identifier().getText(), INCLUDE_DATA, ctx);
    ...
    public static void processIncludeExit(TreeWalkListener listener, IncludeStatementContext ctx) {
        ...
        // Check for stack to be non empty.
        ListenerValidation.checkStackIsNotEmpty(listener, ListenerErrorType.MISSING_HOLDER, INCLUDE_DATA, ctx.identifier().getText(), ListenerErrorLocation.EXIT);
        ...
        if (tmpIncludeNode instanceof YangInclude) {
            ...
            switch (tmpNode.getYangConstructType()) {
                ...
                default:
                    throw new ParserException(ListenerErrorMessageConstruction.constructListenerErrorMessage(ListenerErrorType.INVALID_HOLDER, INCLUDE_DATA,
                            ctx.identifier().getText(),
                            ListenerErrorLocation.EXIT));
            }
        } else {
            throw new ParserException(ListenerErrorMessageConstruction.constructListenerErrorMessage(ListenerErrorType.MISSING_CURRENT_HOLDER, INCLUDE_DATA,
