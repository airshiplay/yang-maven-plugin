...
import static com.airlenet.yang.compiler.datamodel.utils.YangConstructType.IMPORT_DATA;
...
public final class ImportListener {
    ...
    public static void processImportEntry(TreeWalkListener listener, ImportStatementContext ctx) {
        ...
        ListenerValidation.checkStackIsNotEmpty(listener, ListenerErrorType.MISSING_HOLDER, IMPORT_DATA, ctx.identifier().getText(), ListenerErrorLocation.ENTRY);

        String identifier = ListenerUtil.getValidIdentifier(ctx.identifier().getText(), IMPORT_DATA, ctx);
    ...
    public static void processImportExit(TreeWalkListener listener, ImportStatementContext ctx) {
        ...
        // Check for stack to be non empty.
        ListenerValidation.checkStackIsNotEmpty(listener, ListenerErrorType.MISSING_HOLDER, IMPORT_DATA, ctx.identifier().getText(), ListenerErrorLocation.EXIT);
        ...
        if (tmpImportNode instanceof YangImport) {
            ...
            // Check for stack to be non empty.
            ListenerValidation.checkStackIsNotEmpty(listener, ListenerErrorType.MISSING_HOLDER, IMPORT_DATA, ctx.identifier().getText(),
            ...
            switch (tmpNode.getYangConstructType()) {
                ...
                default:
                    throw new ParserException(ListenerErrorMessageConstruction.constructListenerErrorMessage(ListenerErrorType.INVALID_HOLDER, IMPORT_DATA,
                                                                            ctx.identifier().getText(),
                                                                            ListenerErrorLocation.EXIT));
            }
        } else {
            throw new ParserException(ListenerErrorMessageConstruction.constructListenerErrorMessage(ListenerErrorType.MISSING_CURRENT_HOLDER, IMPORT_DATA,
