...
import static com.airlenet.yang.compiler.datamodel.YangDecimal64.MAX_FRACTION_DIGITS_VALUE;
import static com.airlenet.yang.compiler.datamodel.YangDecimal64.MIN_FRACTION_DIGITS_VALUE;
import static com.airlenet.yang.compiler.datamodel.utils.YangConstructType.FRACTION_DIGITS_DATA;
...
public final class FractionDigitsListener {
    ...
    public static void processFractionDigitsEntry(TreeWalkListener listener,
        ...
        ListenerValidation.checkStackIsNotEmpty(listener, ListenerErrorType.MISSING_HOLDER, FRACTION_DIGITS_DATA,
                             ctx.fraction().getText(), ListenerErrorLocation.ENTRY);
        ...
        if (tmpNode instanceof YangType) {
            ...
            if (!decimal64Node.isValidFractionDigit()) {
                throw new ParserException(constructErrorMsg(ctx, ListenerErrorType.INVALID_CONTENT));
            }
        } else {
            throw new ParserException(constructErrorMsg(ctx, ListenerErrorType.INVALID_HOLDER));
        }
    }
    ...
    private static String constructErrorMsg(
        ...
        return ListenerErrorMessageConstruction.constructListenerErrorMessage(type, FRACTION_DIGITS_DATA,
                                             ctx.fraction().getText(), ListenerErrorLocation.ENTRY);
    }
