...
public class YangValidateAnnotationMojo extends AbstractProcessorMojo
{
    public void execute() throws MojoExecutionException
    {
        ...
        try {
            ...
            getLog().info("yang files count = " + yangList.size() + " to be validate");
            ...
            for (String yangfile : yangList) {
                ...
                try {
                    if (pythonUsing) {
                        if(windows){
                            ProcessUtil.process( new File(pythonHome,"Scripts/pyang").getAbsolutePath(), yangfile, "-p", path,"--lax-quote-checks","-W","none");
                        }else{
