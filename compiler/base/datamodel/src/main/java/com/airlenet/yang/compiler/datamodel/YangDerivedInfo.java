...
import com.airlenet.yang.compiler.datamodel.utils.RestrictionResolver;
import com.airlenet.yang.compiler.datamodel.utils.builtindatatype.YangDataTypeUtils;
...
public class YangDerivedInfo<T> extends DefaultLocationInfo
    ...
    public ResolvableStatus resolve() throws DataModelException {
        ...
        if (type == YangDataTypes.DERIVED) {
        ...
        } else if (type == YangDataTypes.LEAFREF || type == YangDataTypes.IDENTITYREF) {
             ...
             */
            if (YangDataTypeUtils.isOfRangeRestrictedType(effectiveBuiltInType)) {
                return getResolveStatusForRangeRestrictionType(extended);
            } else if (effectiveBuiltInType == YangDataTypes.STRING) {
                return getResolveStatusForString(extended);
            } else if (effectiveBuiltInType == YangDataTypes.BINARY) {
                return getResolveStatusForBinary(extended);
            } else if (effectiveBuiltInType == YangDataTypes.DECIMAL64) {
        ...
        if (effectiveBuiltInType.isNonRestrictedType()) {
            if (lengthRes == null && rangeRes == null && (
                ...
                throw new DataModelException(ErrorMessages.getErrorMsg(
    ...
    private ResolvableStatus resolveTypeDerivedInfo(YangType<?> baseType)
        ...
        if (baseType.getResolvableStatus() != ResolvableStatus.INTRA_FILE_RESOLVED &&
                baseType.getResolvableStatus() != ResolvableStatus.RESOLVED) {
            throwError();
        }
        ...
        if (getReferredTypeDef().getTypeDefBaseType()
                .getResolvableStatus() == ResolvableStatus.INTRA_FILE_RESOLVED) {
    ...
    private YangRangeRestriction resolveLengthRestriction(YangRangeRestriction refLr)
         ...
         */
        if (refLr == null) {
            return RestrictionResolver.processLengthRes(null, getLineNumber(), getCharPosition(),
         ...
         */
        YangRangeRestriction curLr = RestrictionResolver.processLengthRes(
    ...
    private void resolveRangeRestriction(YangRangeRestriction refRangeRestriction)
        ...
        if (refRangeRestriction == null) {
            YangRangeRestriction curRangeRestriction =
                    RestrictionResolver.processRangeRestriction(null, getLineNumber(),
         ...
         */
        YangRangeRestriction curRangeRestriction =
                RestrictionResolver.processRangeRestriction(refRangeRestriction, getLineNumber(),
