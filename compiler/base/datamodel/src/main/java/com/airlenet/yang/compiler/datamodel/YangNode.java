...
//        }
    ...
    public YangNode clone(YangUses yangUses, boolean isDeviation, boolean isAnyData)
        ...
        if (clonedNode instanceof YangLeavesHolder) {
            try {
                DataModelUtils.cloneListOfLeaf((YangLeavesHolder) clonedNode, yangUses,
                                isDeviation);
                DataModelUtils.cloneListOfLeafList((YangLeavesHolder) clonedNode, yangUses,
                                    isDeviation);
            } catch (DataModelException e) {
            ...
            while (nextNodeToClone != srcRootNode) {
                ...
                if (curTraversal == TraversalType.CHILD) {
                    ...
                    if (clonedTreeCurNode instanceof YangLeavesHolder) {
                        DataModelUtils.updateClonedLeavesUnionEnumRef((YangLeavesHolder) clonedTreeCurNode);
                    }
    ...
    public void setParentContext() {
        if (this instanceof SchemaDataNode) {
            SchemaContext t = DataModelUtils.getParentSchemaContext(this.getParent());
            if (t != null) {
                parentContext = t;
            }

            // As rpc and anydata node is not leaf holder
            if (nodeType != YangNodeType.RPC_NODE && nodeType != YangNodeType.ANYDATA_NODE) {
