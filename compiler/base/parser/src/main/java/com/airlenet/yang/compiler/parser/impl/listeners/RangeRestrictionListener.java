...
import static com.airlenet.yang.compiler.datamodel.utils.RestrictionResolver.processRangeRestriction;
import static com.airlenet.yang.compiler.datamodel.utils.YangConstructType.RANGE_DATA;
import static com.airlenet.yang.compiler.datamodel.utils.YangConstructType.TYPE_DATA;
import static com.airlenet.yang.compiler.datamodel.utils.builtindatatype.YangDataTypeUtils.isOfRangeRestrictedType;
...
public final class RangeRestrictionListener {
    ...
    public static void processRangeRestrictionExit(TreeWalkListener lis,
        ...
        // Check for stack to be non empty.
        ListenerValidation.checkStackIsNotEmpty(lis, ListenerErrorType.MISSING_HOLDER, RANGE_DATA, txt, ListenerErrorLocation.EXIT);

        Parsable tmpData = lis.getParsedDataStack().peek();
        if (!(tmpData instanceof YangRangeRestriction)) {
            throw new ParserException(ListenerErrorMessageConstruction.constructListenerErrorMessage(
                    ListenerErrorType.MISSING_CURRENT_HOLDER, RANGE_DATA, txt, ListenerErrorLocation.EXIT));
        }
